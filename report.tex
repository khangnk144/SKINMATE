...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{dfd_login.png}
\end{figure}
...
\textbf{ - Trích xuất INCI bằng ảnh OCR}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{inci.png}
\end{figure}
\vspace{0.5cm}

Mô tả luồng dữ liệu:
\begin{itemize}
    \item D1: Ảnh bảng thành phần và yêu cầu trích xuất INCI.
    \item D2: File ảnh tải lên từ thiết bị, định dạng JPG/PNG.
    \item D3: Dữ liệu ảnh tạm hoặc trạng thái form phân tích hiện tại.
    \item D4: Chuỗi INCI đã trích xuất, ghi tạm vào ô nhập liệu của form phân tích.
    \item D5: Chuỗi INCI được tự động điền lên giao diện.
    \item D6: Thông báo kết quả OCR thành công hoặc lỗi ảnh không hợp lệ.
\end{itemize}

Thuật toán:
\begin{enumerate}
    \item Nhận D1 từ người dùng và D2 từ thiết bị nhập.
    \item Kiểm tra định dạng ảnh JPG/PNG.
    \item Gửi ảnh qua OCR để nhận dạng ký tự.
    \item Nếu OCR thất bại, trả D6 là thông báo lỗi.
    \item Nếu OCR thành công, chuẩn hóa chuỗi thành phần.
    \item Ghi D4 vào bộ nhớ tạm/form phân tích.
    \item Trả D5 để tự động điền chuỗi INCI lên giao diện.
    \item Trả D6 cho người dùng.
\end{enumerate}

\textbf{ - Phân tích AI dự phòng — Gemini Fallback}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Screenshot 2026-05-30 220829.png}
\end{figure}
\vspace{0.5cm}

Mô tả luồng dữ liệu:
\begin{itemize}
    \item D1: Thành phần chưa có trong CSDL và loại da của người dùng.
    \item D2: Chuỗi INCI ban đầu hoặc thao tác yêu cầu phân tích.
    \item D3: Dữ liệu hiện có trong Ingredients, IngredientRules.
    \item D4: Thành phần mới và luật phân tích mới được lưu vào CSDL.
    \item D5: Kết quả phân tích từ AI gồm mô tả, nhãn GOOD/BAD/NEUTRAL.
    \item D6: Kết quả phân tích thành phần lạ trả về cho người dùng.
\end{itemize}

Thuật toán:
\begin{enumerate}
    \item Nhận D1 là thành phần chưa tồn tại trong CSDL.
    \item Đọc D3 từ bảng Ingredients và IngredientRules.
    \item Nếu đã có dữ liệu, trả kết quả từ CSDL.
    \item Nếu chưa có dữ liệu, gọi Gemini API để phân tích theo loại da.
    \item Nhận kết quả gồm mô tả và phân loại GOOD/BAD/NEUTRAL.
    \item Lưu D4 vào Ingredients và IngredientRules.
    \item Trả D5 cho thiết bị xuất.
    \item Trả D6 cho người dùng.
\end{enumerate}
...
\textbf{ - Báo cáo phân loại sai thành phần}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Screenshot 2026-05-30 222624.png}
\end{figure}
\vspace{0.5cm}

Mô tả luồng dữ liệu:
\begin{itemize}
    \item D1: Thành phần cần báo cáo, phân loại đề xuất, lý do, link dẫn chứng.
    \item D2: Thao tác nhập form báo cáo và bấm gửi.
    \item D3: Thông tin thành phần hiện tại từ CSDL.
    \item D4: Báo cáo mới ở trạng thái PENDING.
    \item D5: Cập nhật danh sách báo cáo cộng đồng hoặc trạng thái gửi báo cáo.
    \item D6: Thông báo gửi báo cáo thành công hoặc lỗi.
\end{itemize}

Thuật toán:
\begin{enumerate}
    \item Nhận D1 từ người dùng.
    \item Nhận D2 từ thiết bị nhập.
    \item Kiểm tra người dùng đã đăng nhập.
    \item Kiểm tra lý do báo cáo có tối thiểu 20 ký tự.
    \item Đọc D3 để xác định thành phần đang được báo cáo.
    \item Nếu dữ liệu không hợp lệ, trả D6 là thông báo lỗi.
    \item Nếu hợp lệ, lưu D4 vào IngredientReports với trạng thái PENDING.
    \item Trả D5 để cập nhật giao diện.
    \item Trả D6 cho người dùng.
\end{enumerate}


\textbf{ - Bình chọn báo cáo cộng đồng}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Screenshot 2026-05-30 223639.png}
\end{figure}
\vspace{0.5cm}

Mô tả luồng dữ liệu:
\begin{itemize}
    \item D1: Mã báo cáo và loại bình chọn UP/DOWN.
    \item D2: Thao tác bấm nút UP/DOWN hoặc hủy bình chọn.
    \item D3: Báo cáo hiện tại và phiếu bình chọn hiện có của người dùng.
    \item D4: Phiếu bình chọn mới, cập nhật hoặc bị xóa.
    \item D5: Tổng điểm bình chọn mới và trạng thái nút vote.
    \item D6: Kết quả cập nhật bình chọn.
\end{itemize}

Thuật toán:
\begin{enumerate}
    \item Nhận D1 gồm mã báo cáo và loại bình chọn.
    \item Nhận D2 từ thao tác bấm nút.
    \item Kiểm tra người dùng đã đăng nhập.
    \item Đọc D3 từ IngredientReports và ReportVotes.
    \item Nếu người dùng chưa vote, tạo mới D4.
    \item Nếu người dùng đổi vote, cập nhật D4.
    \item Nếu người dùng bấm lại cùng vote, hủy vote hiện tại.
    \item Tính lại tổng điểm.
    \item Trả D5 để cập nhật giao diện.
    \item Trả D6 cho người dùng.
\end{enumerate}
...
\textbf{ - Xem thông báo và đánh dấu đã đọc}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Screenshot 2026-05-30 223957.png}
\end{figure}
\vspace{0.5cm}

Mô tả luồng dữ liệu:
\begin{itemize}
    \item D1: Yêu cầu mở danh sách thông báo hoặc đánh dấu đã đọc.
    \item D2: Thao tác click chuông thông báo, click từng thông báo hoặc “đánh dấu tất cả”.
    \item D3: Danh sách thông báo của người dùng.
    \item D4: Trạng thái isRead = true cho một hoặc nhiều thông báo.
    \item D5: Danh sách thông báo, số lượng chưa đọc, trạng thái đã đọc.
    \item D6: Hiển thị dropdown thông báo hoặc kết quả cập nhật.
\end{itemize}
...
\begin{enumerate}
    \item Nhận D1 từ người dùng.
    \item Nhận D2 từ thiết bị nhập.
    \item Kiểm tra token đăng nhập.
    \item Nếu người dùng mở dropdown, đọc D3 từ Notifications.
    \item Nếu người dùng đánh dấu đã đọc, cập nhật D4.
    \item Tính lại số thông báo chưa đọc.
    \item Trả D5 cho thiết bị xuất.
    \item Trả D6 cho người dùng.
    \item Hệ thống có thể tự tải lại thông báo định kỳ theo cơ chế polling.
\end{enumerate}
...
\textbf{- Duyệt báo cáo cộng đồng}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Screenshot 2026-05-30 225009.png}
\end{figure}

Mô tả luồng dữ liệu:
\begin{itemize}
    \item D1: Mã báo cáo, quyết định APPROVED/REJECTED, ghi chú Admin.
    \item D2: Thao tác chọn báo cáo, nhập ghi chú, bấm duyệt hoặc từ chối.
    \item D3: Báo cáo đang chờ duyệt và luật phân tích hiện tại.
    \item D4: Trạng thái báo cáo đã xử lý, luật phân tích được cập nhật, thông báo gửi cho người báo cáo.
    \item D5: Danh sách báo cáo chờ duyệt sau khi cập nhật.
    \item D6: Thông báo duyệt thành công hoặc lỗi.
\end{itemize}
...
\begin{enumerate}
    \item Nhận D1 từ Admin.
    \item Nhận D2 từ thiết bị nhập.
    \item Kiểm tra quyền Admin.
    \item Đọc D3 từ IngredientReports và IngredientRules.
    \item Nếu báo cáo không còn trạng thái PENDING, trả D6 là lỗi.
    \item Nếu Admin chọn REJECTED, cập nhật trạng thái báo cáo.
    \item Nếu Admin chọn APPROVED, cập nhật trạng thái báo cáo và upsert luật vào IngredientRules.
    \item Tạo thông báo trong Notifications gửi người đã báo cáo.
    \item Trả D5 để làm mới danh sách báo cáo chờ duyệt.
    \item Trả D6 cho Admin.
\end{enumerate}

\textbf{DFD 7 — Gửi thông báo đến người dùng — Admin}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{Screenshot 2026-05-30 225024.png}
\end{figure}

Mô tả luồng dữ liệu:
\begin{itemize}
    \item D1: Người nhận, tiêu đề, nội dung, liên kết điều hướng nếu có.
    \item D2: Dữ liệu nhập từ form Admin hoặc request API.
    \item D3: Thông tin người dùng nhận thông báo.
    \item D4: Thông báo mới loại \texttt{ADMIN\_MESSAGE}.
    \item D5: Kết quả gửi thông báo và cập nhật badge thông báo phía người dùng.
    \item D6: Thông báo gửi thành công hoặc lỗi.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Nhận D2 từ thiết bị nhập.
    \item Kiểm tra quyền Admin.
    \item Đọc D3 từ bảng Users để xác định người nhận.
    \item Nếu người nhận không tồn tại, trả D6 là lỗi.
    \item Tạo D4 trong bảng Notifications.
    \item Trả D5 cho thiết bị xuất.
    \item Trả D6 cho Admin.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{uc_diagram.png}
...
\subsubsection{Sơ đồ trạng thái của AnalysisHistory}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_home.png} % Thay bằng ảnh chụp màn hình thực tế của bạn
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_register.png} % Thay bằng ảnh chụp màn hình thực tế
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_login.png} % Thay bằng ảnh chụp màn hình thực tế
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_analysis.png} % Thay bằng ảnh chụp màn hình thực tế
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_profile.png} % Thay bằng ảnh chụp màn hình thực tế
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_history.png} % Thay bằng ảnh chụp màn hình thực tế
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_admin_dashboard.png} % Thay bằng ảnh chụp màn hình thực tế
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_admin_products.png} % Thay bằng ảnh chụp màn hình thực tế
    \caption{Giao diện Màn hình Quản lý sản phẩm}
    \label{fig:ui_admin_products}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_admin_user.png} % Thay bằng ảnh chụp màn hình thực tế
    \caption{Giao diện Màn hình Quản lý người dùng}
    % \label{fig:ui_admin_products}
\end{figure}
...
\begin{figure}
    ...
    \caption{Màn hình Báo cáo Thống kê}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_im_ex.png}
    \caption{Màn hình nhập xuất dữ liệu}
\end{figure}
\textbf{b) Mô tả các đối tượng trên màn hình}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_popup_detail.png}
    \caption{Popup chi tiết thành phần}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_popup_confirm.png}
    \caption{Popup xác nhận thao tác}
\end{figure}
\textbf{b) Mô tả các đối tượng trên màn hình}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_community_report.png}
    \caption{Màn hình Cộng đồng - Báo cáo}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{ui_duyet_report.png}
    \caption{Màn hình duyệt báo cáo}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{duyet_report_2.png}
    \caption{Màn hình xác nhận từ chối}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{popup_noti.png}
    \caption{Màn hình Popup Thông báo}
\end{figure}
\textbf{b) Mô tả các đối tượng trên màn hình}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{popup_report.png}
    \caption{Popup Báo cáo phân loại}
\end{figure}
...
Sau quá trình phân tích, thiết kế và xây dựng, nhóm đã hoàn thành hệ thống SKINMATE đúng với mục tiêu của đề tài là hỗ trợ người dùng kiểm tra, đánh giá và lựa chọn mỹ phẩm dựa trên bảng thành phần. Hệ thống không chỉ dừng lại ở chức năng tra cứu đơn lẻ mà đã hình thành một nền tảng Web tương đối hoàn chỉnh, kết hợp giao diện người dùng, xử lý nghiệp vụ phía máy chủ, cơ sở dữ liệu và các tích hợp bên ngoài.

\subsubsection{Ưu điểm}

\begin{itemize}
    \item \textbf{Đáp ứng nghiệp vụ cốt lõi:} Hệ thống cho phép người dùng đăng ký, đăng nhập, cập nhật hồ sơ loại da và phân tích chuỗi thành phần mỹ phẩm theo từng loại da. Kết quả phân tích được phân loại theo các mức Tốt, Xấu và Trung bình, giúp người dùng phổ thông dễ hiểu hơn khi đọc bảng thành phần INCI.
    \item \textbf{Kết hợp luật nghiệp vụ và AI:} Cơ chế phân tích chính dựa trên cơ sở dữ liệu thành phần và bảng quy tắc an toàn theo loại da. Khi gặp thành phần chưa có trong dữ liệu, hệ thống có thể sử dụng Gemini AI làm phương án dự phòng và lưu lại kết quả để tái sử dụng, giúp dữ liệu ngày càng đầy đủ hơn.
    \item \textbf{Hỗ trợ nhập liệu linh hoạt:} Ngoài việc nhập thủ công chuỗi thành phần, người dùng có thể tải ảnh nhãn sản phẩm để hệ thống trích xuất danh sách thành phần bằng OCR, giảm thao tác gõ tay và phù hợp hơn với tình huống sử dụng thực tế.
    \item \textbf{Có hệ sinh thái chức năng cho người dùng:} Hệ thống đã triển khai lịch sử phân tích, gợi ý sản phẩm an toàn theo loại da, màn hình cộng đồng để bình chọn báo cáo phân loại và popup thông báo trong ứng dụng.
    \item \textbf{Có hệ thống quản trị tương đối đầy đủ:} Admin có thể quản lý thành phần, quy tắc an toàn, sản phẩm, người dùng, báo cáo thống kê, nhập/xuất dữ liệu Excel và duyệt báo cáo cộng đồng. Khi báo cáo được duyệt, hệ thống tự động cập nhật quy tắc và gửi thông báo cho người gửi báo cáo.
    \item \textbf{Kiến trúc rõ ràng, dễ mở rộng:} Dự án được tách thành Frontend Next.js và Backend Express.js theo mô hình phân lớp Route -- Middleware -- Controller -- Service -- Prisma. Cơ sở dữ liệu PostgreSQL được quản lý bằng Prisma, giúp định nghĩa quan hệ dữ liệu rõ ràng và giảm lỗi truy vấn thủ công.
    \item \textbf{Chú trọng bảo mật và độ ổn định:} Hệ thống sử dụng JWT để xác thực, bcrypt để băm mật khẩu, middleware phân quyền Admin, kiểm tra tài khoản bị khóa, giới hạn số lượt phân tích, giới hạn dung lượng request và ràng buộc dữ liệu ở mức cơ sở dữ liệu.
\end{itemize}

\subsubsection{Nhược điểm}

\begin{itemize}
    \item \textbf{Chất lượng phân tích phụ thuộc vào dữ liệu:} Với các thành phần hoặc quy tắc chưa được quản trị đầy đủ, kết quả có thể chỉ ở mức Trung bình hoặc phải phụ thuộc vào AI fallback.
    \item \textbf{Phụ thuộc dịch vụ bên ngoài:} Các chức năng Gemini AI và OCR cần API key và kết nối Internet ổn định; khi dịch vụ bên ngoài lỗi, hệ thống phải chuyển sang cơ chế dự phòng.
    \item \textbf{Chưa phải hệ thống tư vấn y khoa:} Kết quả phân tích chỉ hỗ trợ tham khảo theo dữ liệu và quy tắc đã xây dựng, chưa thể thay thế tư vấn của chuyên gia da liễu trong các trường hợp bệnh lý hoặc dị ứng nghiêm trọng.
    \item \textbf{Dữ liệu sản phẩm còn cần mở rộng:} Chức năng gợi ý sản phẩm đã có nền tảng xử lý, nhưng độ hữu ích thực tế phụ thuộc vào số lượng sản phẩm và thành phần được Admin nhập vào hệ thống.
\end{itemize}

\subsection{Hướng phát triển}

Để SKINMATE có thể ứng dụng rộng rãi và chuyên nghiệp hơn, nhóm đề xuất một số hướng phát triển sau:

\begin{itemize}
    \item \textbf{Mở rộng cơ sở dữ liệu thành phần và sản phẩm:} Bổ sung thêm dữ liệu INCI, mô tả khoa học, nguồn tham khảo và danh mục sản phẩm phổ biến tại thị trường Việt Nam.
    \item \textbf{Nâng cao độ chính xác của phân tích:} Hoàn thiện bộ quy tắc theo từng loại da, bổ sung mức độ rủi ro theo nồng độ hoặc vị trí thành phần trong danh sách INCI, đồng thời cải tiến cơ chế kiểm chứng dữ liệu do AI sinh ra.
    \item \textbf{Cải thiện OCR và trải nghiệm quét ảnh:} Tối ưu khả năng nhận diện ảnh nhãn sản phẩm trong điều kiện ánh sáng, góc chụp và bố cục khác nhau; bổ sung bước cho phép người dùng chỉnh sửa danh sách thành phần trước khi phân tích.
    \item \textbf{Phát triển tính năng cá nhân hóa sâu hơn:} Cho phép người dùng lưu danh sách thành phần cần tránh, ghi nhận phản ứng da sau khi dùng sản phẩm và nhận gợi ý phù hợp hơn với lịch sử cá nhân.
    \item \textbf{Tăng cường quản trị cộng đồng:} Bổ sung cơ chế uy tín người dùng, lọc báo cáo trùng lặp, lưu vết thay đổi quy tắc và hiển thị nguồn tham khảo rõ ràng hơn khi Admin duyệt báo cáo.
    \item \textbf{Hoàn thiện triển khai thực tế:} Chuẩn hóa môi trường production, bổ sung CI/CD, logging, giám sát lỗi và các bài kiểm thử tự động cho những luồng nghiệp vụ quan trọng.
\end{itemize}

\subsection{Tổng kết chung}

Đồ án SKINMATE là cơ hội để nhóm áp dụng các kiến thức của môn Nhập môn Công nghệ Phần mềm vào một bài toán thực tế có ý nghĩa với người dùng: hỗ trợ lựa chọn mỹ phẩm an toàn và phù hợp hơn với từng loại da. Thông qua quá trình xây dựng hệ thống, nhóm đã thực hành đầy đủ các bước từ khảo sát nhu cầu, phân tích yêu cầu, thiết kế cơ sở dữ liệu, xây dựng giao diện, phát triển API, kiểm thử đến hoàn thiện các chức năng quản trị.

Mặc dù sản phẩm vẫn còn những hạn chế về độ bao phủ dữ liệu, khả năng phụ thuộc vào dịch vụ bên ngoài và mức độ kiểm chứng chuyên môn, hệ thống hiện tại đã tạo được nền tảng vững chắc cho một ứng dụng phân tích thành phần mỹ phẩm có thể tiếp tục mở rộng. Quan trọng hơn, nhóm hiểu rõ hơn vai trò của việc phân tích yêu cầu, thiết kế kiến trúc, phân chia module và phối hợp giữa các thành viên trong quá trình phát triển phần mềm.
